(function() {
	...
	function q(a) {
		a = a.split(".");
		for (var b = n, c; c = a.shift();) if (b[c] != h) b = b[c];
		else return h;
		return b
	}
	...
	function fa(a, b) {
		...
		t: {
			c = a.length;
			for (var d = s(a) ? a.split("") : a, e = 0; e < c; e++) if (e in d && b.call(g, d[e], e, a)) {
				c = e;
				break t
			}
			c = -1
		}
		return 0 > c ? h : s(a) ? a.charAt(c) : a[c]
	}
	function ga(a) {
	...
	if (G = ka()) {
		var la = n.navigator;
		B = 0 == G.indexOf("Opera");
		C = !B && -1 != G.indexOf("MSIE");
		D = !B && -1 != G.indexOf("WebKit");
		F = !B && !D && "Gecko" == la.product
	}
	var I = C,
		J = F,
		ma = D;
	...
	t: {
		var L = "",
			M;
		if (B && n.opera) var N = n.opera.version,
			L = "function" == typeof N ? N() : N;
		else if (J ? M = /rv\:([^\);]+)(\)|;)/ : I ? M = /MSIE\s+([^\);]+)(\)|;)/ : ma && (M = /WebKit\/(\S+)/), M) var oa = M.exec(ka()),
			L = oa ? oa[1] : "";
		if (I) {
			var pa = na();
			if (pa > parseFloat(L)) {
		...
		if (!ra[a]) {
			for (var b = 0, c = String(qa).replace(/^[\s\xa0]+|[\s\xa0]+$/g, "").split("."), d = String(a).replace(/^[\s\xa0]+|[\s\xa0]+$/g, "").split("."), e = Math.max(c.length, d.length), f = 0; 0 == b && f < e; f++) {
				...
				do {
					...
					b = ((0 == w[1].length ? 0 : parseInt(w[1], 10)) < (0 == x[1].length ? 0 : parseInt(x[1], 10)) ? -1 : (0 == w[1].length ? 0 : parseInt(w[1], 10)) > (0 == x[1].length ? 0 : parseInt(x[1], 10)) ? 1 : 0) || ((0 == w[2].length) < (0 == x[2].length) ? -1 : (0 == w[2].length) > (0 == x[2].length) ? 1 : 0) || (w[2] < x[2] ? -1 : w[2] > x[2] ? 1 : 0)
				} while (0 == b)
			}
			ra[a] = 0 <= b
		}
	...
	var ta = n.document,
		ua = !ta || !I ? g : na() || ("CSS1Compat" == ta.compatMode ? parseInt(qa, 10) : 5);
	if (J || I) {
		var O;
		if (O = I) O = I && 9 <= ua;
		O || J && sa("1.9.1")
	...
	function va() {
		...
		for (b = c = 0; a = e[b]; b++) {
			var f = a.className;
			"function" == typeof f.split && 0 <= ea(f.split(/\s+/), "yt-player") && (d[c++] = a)
		}
		d.length = c;
		return d
	}
...
-\x1f]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g, "]").replace(/(?:^|:|,)(?:[\s\u2028\u2029]*\[)+/g, ""))) try {
			return eval("(" + a + ")")
		} catch (b) {}
	...
	function P(a, b, c) {
		...
		case "object":
			...
			if ("array" == r(b)) {
				var d = b.length;
				c.push("[");
				for (var e = "", f = 0; f < d; f++) c.push(e), P(a, b[f], c), e = ",";
				c.push("]");
				break
			}
			c.push("{");
			d = "";
			for (e in b) Object.prototype.hasOwnProperty.call(b, e) && (f = b[e], "function" != typeof f && (c.push(d), za(e, c), c.push(":"), P(a, f, c), d = ","));
			c.push("}");
			break;
		case "function":
			break;
	...
	function Ga(a) {
		if (Ha) {
			...
			if (b) {
				var c = b.href;
				if (c && (c = (c = Ga(c)[3] || h) && decodeURIComponent(c)) && c != b.hostname) throw Ha = !0, Error();
			}
		}
		return a.match(Fa)
	}
	...
	function Ia(a, b, c) {
		if ("array" == r(b)) for (var d = 0; d < b.length; d++) Ia(a, String(b[d]), c);
		else b != h && c.push("&", a, "" === b ? "" : "=", encodeURIComponent(String(b)))
	};

	function Ja(a, b) {
		return a.dataset ? a.dataset[Ka(b)] : a.getAttribute("data-" + b)
	}
	var La = {};
	...
	var Ma = q("yt.dom.getNextId_");
	if (!Ma) {
		Ma = function() {
			return ++Na
		};
		u("yt.dom.getNextId_", Ma);
		var Na = 0
	};
	...
	var Oa = window.yt && window.yt.intervals_ || [];
	u("yt.intervals_", Oa);
	...
	function Ra(a) {
		if (a = a || window.event) {
			for (var b in a) b in Sa || (this[b] = a[b]);
			this.scale = a.scale;
	...
	var A = q("yt.events.listeners_") || {};
	u("yt.events.listeners_", A);
	var Ta = q("yt.events.counter_") || {
		count: 0
	};
	u("yt.events.counter_", Ta);

	function Ua(a, b, c) {
		return ja(function(d) {
			return d[0] == a && d[1] == b && d[2] == c && d[4] == k
		})
	}
	...
	function Va(a, b, c) {
		if (a && (a.addEventListener || a.attachEvent)) {
			...
			if (!d) {
				...
				f = Qa(f);
				A[d] = [a, b, c, f, k];
				a.addEventListener ? "mouseenter" == b && e ? a.addEventListener("mouseover", f, k) : "mouseleave" == b && e ? a.addEventListener("mouseout", f, k) : "mousewheel" == b && "MozBoxSizing" in document.documentElement.style ? a.addEventListener("MozMousePixelScroll", f, k) : a.addEventListener(b, f, k) : a.attachEvent("on" + b, f)
			}
		}
	}
	function Wa(a) {
	...
	function T(a, b) {
		...
		this.d = this.id = 0;
		this.k = this.h = h;
		var c = s(a) ? document.getElementById(a) : a;
		if (c) {
			if ("iframe" != c.tagName.toLowerCase()) {
				for (var d = c, e = document.createElement("iframe"), d = d.attributes, f = 0, p = d.length; f < p; f++) {
					var l = d[f].value;
					l != h && ("" != l && "null" != l) && e.setAttribute(d[f].name, l)
				}
				e.setAttribute("frameBorder", 0);
				e.setAttribute("allowfullscreen", 1);
				e.setAttribute("title", "YouTube " + U(this.b, "title"));
				(d = U(this.b, "width")) && e.setAttribute("width", d);
				(d = U(this.b, "height")) && e.setAttribute("height", d);
				d = this.p();
				d.enablejsapi = window.postMessage ? 1 : 0;
				window.location.host && (d.origin = window.location.protocol + "//" + window.location.host);
				var f = U(this.b, "host") + this.q() + "?",
					p = [],
					E;
				for (E in d) Ia(E, d[E], p);
				p[0] = "";
				e.src = f + p.join("");
				this.i = c;
				(E = c.parentNode) && E.replaceChild(e, c);
				c = e
			}
			this.a = c;
			this.id = this[aa] || (this[aa] = ++ba);
			this.a.id && (Xa[this.a.id] = this);
			if (window.postMessage) {
				this.h = new R;
				ab(this);
				var c = U(this.b, "events"),
					H;
				for (H in c) c.hasOwnProperty(H) && this.addEventListener(H, c[H])
			}
		}
	}
	m = T.prototype;
	m.K = function() {
		this.a.id && (Xa[this.a.id] = h);
		var a = this.h;
	...
	function ab(a) {
		bb(a.b, a, a.id);
		a.d = Pa(t(a.t, a));
		Va(a.a, "load", t(function() {
			window.clearInterval(this.d);
			this.d = Pa(t(this.t, this))
		}, a))
	}
	m.p = function() {
		return {}
	...
	m.t = function() {
		this.a && this.a.contentWindow ? cb(this, {
			event: "listening"
		}) : window.clearInterval(this.d)
	};
	m.A = function(a) {
	...
	function cb(a, b) {
		...
		e && (l += e + ":");
		p && (l += "//", f && (l += f + "@"), l += p, d && (l += ":" + d));
		a.a.contentWindow.postMessage(c, l)
	}
	m.M = function(a, b) {
		this.a.width = a;
		this.a.height = b;
	...
	var eb = "corp.google.com youtube.com youtube-nocookie.com prod.google.com sandbox.google.com docs.google.com drive.google.com play.google.com".split(" ");
	var fb = window.YTConfig || {};
	...
	function U(a, b) {
		for (var c = [a.b, fb, a.a], d = 0; d < c.length; d++) {
			var e = c[d][b];
			if (e != g) return e
		}
		return h
	...
	function bb(a, b, c) {
		V || (V = {}, Va(window, "message", t(a.d, a)));
		V[c] = b
	}
	X.prototype.d = function(a) {
		if (a.origin == U(this, "host") || RegExp("^https?://([a-z0-9-]{1,63}\\.)*(" + eb.join("|").replace(/\./g, ".") + ")(:[0-9]+)?([/?#]|$)", "i").test(a.origin)) {
			var b;
			try {
				b = xa(a.data)
			} catch (c) {
	...
	function hb(a) {
		if ("iframe" != a.tagName.toLowerCase()) {
			var b = a.getAttribute("videoid") || Ja(a, "videoid");
			if (b) {
				var c = a.getAttribute("width") || Ja(a, "width"),
					d = a.getAttribute("height") || Ja(a, "height");
				new Y(a, {
					videoId: b,
					width: c,
					height: d
	...
	m.A = function(a) {
		...
		case "onStateChange":
			...
			Z(this, a);
			this.g("onStateChange", b);
			db[b] && this.g(db[b]);
			break;
		case "onPlaybackQualityChange":
			Z(this, a);
			this.g("onPlaybackQualityChange", this.o.playbackQuality);
			break;
		case "onPlaybackRateChange":
			Z(this, a);
			this.g("onPlaybackRateChange", this.o.playbackRate);
			break;
		case "onNewLicenseSession":
			this.g("onNewLicenseSession", a.sessionInfo);
			break;
		case "onError":
			this.g("onError", a.error);
			break;
		case "onApiChange":
			a = a.info || {};
		...
		case "initialDelivery":
			this.o = {};
			this.j = {};
			ib(this, a.apiInterface);
			Z(this, a);
			break;
		case "onFullScreenToggleRequest":
			this.g("onFullScreenToggleRequest");
			break;
		default:
			Z(this, a), this.g(a.event, a.info)
		}
	};
	...
	m.I = function(a) {
		return !this.j.namespaces ? [] : !a ? this.j.namespaces || [] : this.j[a].options || []
	};
	m.H = function(a, b) {
	...
	m.B = function(a) {
		W(this, "setParentVideoId", arguments)
	};
	m.G = function(a) {
	...
	u("YT.Player", Y);
	u("YT.UploadWidget", $);
	T.prototype.destroy = T.prototype.K;
	T.prototype.setSize = T.prototype.M;
	T.prototype.getIframe = T.prototype.L;
	T.prototype.addEventListener = T.prototype.addEventListener;
	Y.prototype.getVideoEmbedCode = Y.prototype.J;
	Y.prototype.getOptions = Y.prototype.I;
	Y.prototype.getOption = Y.prototype.H;
	$.prototype.setParentVideoId = $.prototype.B;
	$.prototype.setVideoDescription = $.prototype.C;
	$.prototype.setVideoKeywords = $.prototype.D;
	$.prototype.setVideoPrivacy = $.prototype.F;
	$.prototype.setVideoTitle = $.prototype.G;
	var kb = q("onYTReady");
	kb && kb();
	var lb = q("onYouTubeIframeAPIReady");
	lb && lb();
	var mb = q("onYouTubePlayerAPIReady");
	mb && mb();
	Ya.push(function() {
		var a = ha(document.getElementsByTagName("yt:player")),
			b;
		b = document;
		b = b.querySelectorAll && b.querySelector ? b.querySelectorAll(".yt-player") : b.getElementsByClassName ? b.getElementsByClassName("yt-player") : va();
		b = ha(b);
		z(ga(a, b), hb)
	});
	Za();
